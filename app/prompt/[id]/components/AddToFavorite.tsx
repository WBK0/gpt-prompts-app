...
const AddToFavorite = ({favorites, id, isFavorite} : {favorites : number, isFavorite: boolean, id: string}) => {
  const handleSubmit = async () => {
    const response = await fetch(`http://localhost:3000/api/prompt/${id}/${isFavorite ? 'dislike' : 'like'}`, {
      method: 'PATCH'
    })
  }
  ...
  return (
    <div className="px-6 py-4 bg-gray-100 flex items-center">
      <button className="bg-gray-200 hover:bg-gray-300 rounded-full px-3 py-1" onClick={handleSubmit}>
        <svg className={`h-5 w-5 fill-current ${isFavorite ? 'text-red-500' : 'text-gray-500'}`} viewBox="0 0 24 24">
      ...
      <span className="text-gray-500 font-gilroyBold ml-2">{favorites}</span>
    </div>
  )
}
